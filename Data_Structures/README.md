...
Queue put 5: 2 3 5 0 0 | front : 0 | rear : 2
print get(): 2  | fornt : 1 | rear : 2
```
queue를 초기화하고, put 함수를 불렀을때 배열에 순서대로 값이 입력되고 이 배열의 값들을 출력합니다 그리고 get함수를 불렀을때 출력되는 값을 보여주고 front와 rear의 값도 함께 출력 하면 됩니다 

# 구현방법

queue는 버스정류장에서 버스를 기다리는 사람들을 생각 해보면 이해가 쉽습니다.
버스가 도착을 하고 줄을 서있는 사람들이 좌석이 있는 만큼 탑승을 하게 되고 나중에 탑승을 위해 온 사람들은 줄의 맨끝에 서게 되는 것과 비슷합니다

queue에 값이 들어올때는 put 함수를 이용해서 값이 순서대로 할당되고, get 함수를 호출 했을때는 배열안에 있는 제일 첫번째 요소가 출력됩니다 

# 코드
[queue](./pointerQ.c)

# 코드설명 
초기화를 하기위해서는 인자로 배열, 들어올 자리를 가르키는 rear와 front를 갖습니다
for문을 통해서 배열의 크기만큼 0을 할당해주고, rear와 front에 unsinged값을 할당해줍니다

배열의 상태를 확인 하기위해 출력함수를 만들어 주고 배열의 값들과 rear, front 값을 출력해줍니다 

put함수는 초기화를 put함수를 호출할때는 rear에 0(배열의 맨앞)을 할당해주고, 이미 값이 들어있는 경우에는 rear를 1증가 시켜 줍니다. 그리고 rear에 자리에 인자로 들어온 item을 할당해줍니다. 배열에서 값을 get함수를 이용해서 출력을 할때 출력될 값을 가르키는 front도 배열에 값이 한개가 들어왔는 때에 그 값을 가르키도록 front가 초기값일대 fornt에 0을 할당해 줍니다.

get함수는 front가 가르키는 값을 반환해줍니다. get함수안에 선언된 변수에 배열의 front번째 값을 할당해주고 fornt를 1증가 시켜줍니다. 그리고 그변수를 반환합니다 
